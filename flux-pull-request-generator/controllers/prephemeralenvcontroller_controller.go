...
import (
	...
	fluxprgeneratorv1alpha1 "github.com/cse-labs/platformops-ephemeral-test-env/api/v1alpha1"
	fluxhelmrelease "github.com/fluxcd/helm-controller/api/v2beta1"
)

// FluxPullRequestGeneratorReconciler reconciles a FluxPullRequestGenerator object
type FluxPullRequestGeneratorReconciler struct {
	client.Client
	Scheme              *runtime.Scheme
	Record              record.EventRecorder
	GHPRRepo            fluxprgeneratorv1alpha1.GithubPRRepository
	GHPATToken          string
	EnvCreationHelmRepo fluxprgeneratorv1alpha1.EnvCreationHelmRepo
}

func (r *FluxPullRequestGeneratorReconciler) getEnvHealthCheckUrl(urlTemplate string, prNumber int, prHeadSHA string) string {
...
func (r *FluxPullRequestGeneratorReconciler) getGHToken(ctx context.Context, prController fluxprgeneratorv1alpha1.FluxPullRequestGenerator) (string, error) {
...
// the FluxPullRequestGenerator object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.12.2/pkg/reconcile
func (r *FluxPullRequestGeneratorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	var prController fluxprgeneratorv1alpha1.FluxPullRequestGenerator
...
func (r *FluxPullRequestGeneratorReconciler) SetupWithManager(mgr ctrl.Manager) error {
	r.Record = mgr.GetEventRecorderFor("pr-ephem-env-controller-controller")
	return ctrl.NewControllerManagedBy(mgr).
		For(&fluxprgeneratorv1alpha1.FluxPullRequestGenerator{}).
		Complete(r)
}
